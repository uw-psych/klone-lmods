...
APP_NAME="${APP_NAME:-rclone}"
GH_REPO="rclone/rclone"
...
if [[ -z "${GH_RELEASE_VERSION:-}" ]]; then
	GH_RELEASE_VERSION="$(python3 -I -c 'import json; j=json.load(open("release.json","r")); print(j["tag_name"])' 2>/dev/null || { echo >&2 "Failed to get download URL"; exit 1; })"
fi

GH_ASSET="rclone-${GH_RELEASE_VERSION}-linux-amd64.zip"

GH_ASSET_URL="$(python3 -I -c 'import json; j=json.load(open("release.json","r")); print([a["browser_download_url"] for a in j["assets"] if a["name"] == "'"${GH_ASSET}"'"][0])' 2>/dev/null)" || { echo >&2 "Failed to get latest version"; exit 1; }
...
unzip -q "${DLTMP}/${GH_ASSET}" -d "${DLTMP}/${APP_NAME}" || { echo >&2 "Failed to extract ${GH_ASSET}"; exit 1; }
