...
APP_NAME="${APP_NAME:-autorestic}"
GH_REPO="cupcakearmy/autorestic"
...
GH_ASSET="autorestic_${GH_RELEASE_VERSION#v}_linux_amd64.bz2"
...
umask 002 && mkdir -p "${APP_MODULE_DIR}" "${APP_INSTALL_DIR}/bin" && rsync -lHP --chmod=a+rwX "${DLTMP}/${APP_NAME}" "${APP_INSTALL_DIR}/bin/" && rsync -lHP --chmod=a+rwX "${SCRIPT_DIR}/modulefile.lua" "${APP_MODULE_DIR}/${APP_VERSION}.lua" && echo "Module ${GROUP_NAME}/${APP_NAME}/${APP_VERSION} installed."
